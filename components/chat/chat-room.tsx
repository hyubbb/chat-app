...
export const ChatRoom = ({ chatId}: { chatId: string }) => {
  ...
  useEffect(() => {
    ...
    // 채팅방참여
    const handle = async () => {
      await axios.post(`/api/socket/chat/${chatId}/join`, {
        userId: user?.userId,
  ...
  const handleLeaveRoom = async () => {
    const {data} = await axios.delete(`/api/socket/chat/${chatId}`, {
      params: {
        userId: user?.userId,
      },
    });
    ...
    <>
      <div className="flex items-center justify-between space-x-2 border-b bg-white p-4 dark:bg-zinc-800 dark:text-zinc-300">
        <div className="flex items-center gap-x-2">
          ...
          {/* <h2 className="font-semibold">{userList?.room_name}</h2>
          {userList?.active_users! > 0 && (
            <span className="text-sm text-gray-500">
              {`${userList?.active_users}명 참여중`}
            </span>
          )} */}
        </div>
